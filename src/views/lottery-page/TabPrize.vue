  ...
  <ion-segment-content id="tabPrize">
    ...
    <ion-content :scrollY="true" class="h-[calc(100%-56px)]">
      ...
      <ion-item v-for="item in giftList.data" :key="item.id">
        ...
        <ion-button class="w-14 h-10 ml-2" color="warning" @click="$emit('add-wish', item)"
    ...
    <!-- 新建奖品弹窗 -->
    <ion-modal :is-open="isNewGiftOpen" @willDismiss="closeNewGift">
      <ion-header>
        <ion-toolbar>
          <ion-title>新建奖品</ion-title>
          <ion-buttons slot="start">
            <ion-button @click="closeNewGift">
      ...
      <ion-footer>
        <ion-toolbar class="ion-padding">
          <div class="flex gap-2 w-full">
            <ion-button fill="outline" class="w-20" @click="closeNewGift">取消</ion-button>
            <ion-button color="primary" class="flex-1" @click="saveNewGift">
              确定
            </ion-button>
          </div>
        </ion-toolbar>
        ...
        <div class="p-4">
          ...
          <ion-list lines="full" class="mt-2 w-full">
            <ion-item v-for="(cate, idx) in manageCatList" :key="cate.id ?? 'new-' + idx" mode="ios">
              <div class="flex flex-col items-stretch bg-gray-100 w-full ">
...
function openNewGift() {
  ...
  isNewGiftOpen.value = true;
  newGiftPreview.value = "";
  newGiftFile.value = null;
}
function closeNewGift() {
  isNewGiftOpen.value = false;
}
async function saveNewGift() {
  ...
  try {
    // 1. 若有选图：先上传图片，接口返回服务端随机生成的文件名（基于时间）
    let imageName = newGift.value.image;
    if (newGiftFile.value) {
      const resp = await uploadPic(newGiftFile.value);
      imageName = resp.filename;
    }

    // 2. 用上传后的文件名创建 gift 对象
    ...
    EventBus.$emit(C_EVENT.TOAST, "添加奖品成功");
    isNewGiftOpen.value = false;
    newGiftFile.value = null;
    newGiftPreview.value = "";
    emit("refresh", { target: { complete: () => { } } });
...
function getGiftImgUrl(item: { img?: string; image?: string }) {
  ...
  return getPicDisplayUrl(raw, 96, 96);
}
</script>
