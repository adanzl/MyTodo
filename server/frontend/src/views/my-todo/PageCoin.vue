...
import type { CoinHistory } from "@/api/api-coin";
...
const refreshRecordList = async (userId: number, pageNum: number, pageSize: number) => {
  ...
  try {
    ...
    const response = await getList<CoinHistory>("t_coin_history", filterParam, pageNum, pageSize);
...
const onRowClick = (_row: CoinHistory) => {};
