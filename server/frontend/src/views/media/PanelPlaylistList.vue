  ...
  <div class="w-64 min-w-62 border rounded p-3 flex flex-col">
    <div class="flex items-center justify-between mb-3">
      ...
      <div class="flex items-center gap-0">
        ...
        <el-button type="success" size="small" @click="$emit('create')" :disabled="dragMode"
          :title="dragMode ? '排序模式下禁用' : '新建播放列表'" class="w-7! h-6! p-0!">
          <el-icon>
            <Plus />
          </el-icon>
        </el-button>
    ...
    <div v-if="playlistCollection && playlistCollection.length > 0" class="flex-1 overflow-y-auto space-y-2 pr-1">
      <div v-for="playlist in playlistCollection" :key="playlist.id"
        ...
        :class="[
          ...
          { 'select-none': true },
        ]" :draggable="dragMode" @click="dragMode ? null : $emit('select', playlist.id)"
        @dragstart="(e: DragEvent) => $emit('drag-start', e, playlist.id)"
        @dragend="(e: DragEvent) => $emit('drag-end', e)" @dragover.prevent="(e: DragEvent) => $emit('drag-over', e)"
        @dragleave="handleDragLeave" @drop.prevent="(e: DragEvent) => $emit('drop', e, playlist.id)">
        <!-- 第一行：名称、曲目数量、功能按钮 -->
        <div class="flex items-center justify-between gap-2">
          ...
          <div class="flex items-center gap-2 shrink-0">
            ...
            <div class="flex items-center justify-center">
              <el-dropdown trigger="click" @command="(command: string) => $emit('menu-command', command, playlist.id)"
                @click.stop>
                <el-button type="default" size="small" plain circle class="w-5! h-5! min-w-5! p-0! text-xs" title="更多">
        ...
        <div class="text-[10px] text-blue-600 min-h-3.5 flex items-center justify-between">
          ...
          <span v-if="playlist.trigger_button" class="text-gray-500 flex items-center gap-1">
            <el-icon class="w-3! h-4!">
              <Cpu />
            </el-icon> {{ playlist.trigger_button }}
          </span>
        </div>
      </div>
    </div>
