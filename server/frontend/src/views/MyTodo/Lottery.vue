              ...
              <div class="flex items-center w-24">
                <template v-if="!row.img">
                  <el-upload
                    ...
                    :on-change="(file: any) => handleImageChange(file, row)"
                  >
...
const lotteryCatPopList = ref<any[]>([]);
...
const refreshGiftList = async (cateId: number, pageNum: number, pageSize: number) => {
  ...
  try {
    ...
    const response = await getList("t_gift", filter, pageNum, pageSize);
    if (response && response.data) {
...
const handleGiftCatChange = (event: any, item: any) => {
  item.cate_id = event;
};

const handleGiftDel = async (item: any) => {
  try {
    item.edited = false;
...
const handleGiftCancel = async (item: any, idx: number) => {
  if (item.id === -1) {
...
const handleGiftSave = async (item: any) => {
  try {
    const data = {
      id: item.id,
      name: item.name,
...
const handleImageChange = async (file: any, row: any) => {
  if (file) {
    try {
...
const handleCateBlur = (item: any, key: string, _idx: number) => {
  console.log("handleCateBlur", item, key);
};

const handleCateSave = async (item: any, _idx: number) => {
  try {
    const data = {
      id: item.id,
      name: item.name,
...
const handleCateDelete = async (item: any) => {
  try {
...
const handleCateEdit = (item: any) => {
  item.edited = true;
};

const handleCateCancel = (item: any, _idx: number) => {
  if (item.id === -1) {
    item.name = "";
    item.cost = 0;
  } else {
