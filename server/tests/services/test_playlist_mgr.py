...
def create_playlist_data(p_id,
    ...
    data = {
        ...
        "pre_lists":
        pre_lists if pre_lists is not None else [[] for _ in range(7)],
...
def mock_device():
    ...
    device.get_status.return_value = (0, {
        "state": "STOPPED",
        "duration": "00:00:10",
        "position": "00:00:10"
    })
...
def playlist_mgr(monkeypatch, mock_device):
    ...
    monkeypatch.setattr('core.device.dlna.upnpclient.Device',
    ...
    monkeypatch.setitem(pm.PlaylistScheduling.refresh_cron_job.__globals__,
                        "scheduler_mgr", mock_scheduler_mgr)

    # Disable background workers by default; specific tests can call internal methods directly
    monkeypatch.setattr(pm.PlaylistRepository, "start_save_worker",
                        lambda self: None)
...
def test_reload_from_rds(playlist_mgr):
    mock_playlist = create_playlist_data("p1", "Playlist 1",
                                         [{
                                             "uri": "file1.mp3"
                                         }])
    mock_playlist_data = {"p1": mock_playlist}
    rds_mgr.set("schedule_play:playlist_collection",
                json.dumps(mock_playlist_data))
...
@patch.object(pm.DurationFetcher, 'update_file_duration', return_value=180)
def test_play_rejects_when_already_playing_unless_force(
        mock_get_duration, playlist_mgr):
    playlist_id = "p1"
    playlist_data = create_playlist_data(playlist_id, "Test Playlist",
                                         [{
                                             "uri": "s1.mp3"
...
def test_get_playlist_with_play_state_in_pre_files(playlist_mgr):
    ...
    playlist_mgr._play_state[playlist_id] = {
        "in_pre_files": True,
        "pre_index": 2
    }
...
@patch.object(pm.DurationFetcher, 'update_file_duration', return_value=180)
def test_play_device_play_failure(mock_get_duration, playlist_mgr,
                                  mock_device):
    playlist_id = "p1"
    playlist_data = create_playlist_data(playlist_id, "Test Playlist",
                                         [{
...
@patch.object(pm.DurationFetcher, 'update_file_duration', return_value=180)
def test_play_sets_volume_when_configured(mock_get_duration, playlist_mgr,
                                          mock_device):
    playlist_id = "p1"
...
@patch.object(pm.DurationFetcher, 'update_file_duration', return_value=180)
def test_play_sets_volume_failure_is_ignored(mock_get_duration, playlist_mgr,
                                             mock_device):
    playlist_id = "p1"
    playlist_data = create_playlist_data(playlist_id,
                                         "Test Playlist", [{
...
@patch.object(pm.DurationFetcher, 'update_file_duration', return_value=180)
def test_play_sets_volume_exception_is_ignored(mock_get_duration, playlist_mgr,
                                               mock_device):
    playlist_id = "p1"
    playlist_data = create_playlist_data(playlist_id,
                                         "Test Playlist", [{
...
def test_collect_files_without_duration(playlist_mgr):
    ...
    files_to_fetch = playlist_mgr._duration_fetcher.collect_files_without_duration(
        p1)
...
def test_update_files_duration(playlist_mgr):
    ...
    updated_count = playlist_mgr._duration_fetcher.update_files_duration(
        p1, durations)
...
def test_start_file_timer_handles_get_status_error(playlist_mgr, mock_device,
                                                   monkeypatch):
    captured = {}
    ...
    monkeypatch.setattr(playlist_mgr, "play_next",
                        MagicMock(return_value=(0, "ok")))
...
def test_start_playlist_duration_timer_when_existing_job_expired_is_removed(
        playlist_mgr, monkeypatch):
    ...
    expired_job = _Job(datetime.datetime.now() -
...
def test_start_batch_duration_fetch_updates_and_blacklists(
        playlist_mgr, monkeypatch):
    ...
    monkeypatch.setitem(df_globals, "get_media_duration",
                        fake_get_media_duration)

    q = MagicMock()
    playlist_mgr._duration_fetcher._rds_save_queue = q

    playlist_mgr._duration_fetcher.start_batch_fetch(
        {"p1": playlist_mgr._playlist_raw["p1"]})
...
def test_save_playlist_to_rds_exception(playlist_mgr, monkeypatch):
    ...
    monkeypatch.setattr(rds_mgr, 'set',
                        MagicMock(side_effect=RuntimeError("redis error")))
...
def test_update_single_playlist_exception(playlist_mgr, monkeypatch):
    ...
    playlist_mgr._playlist_raw = {}
    monkeypatch.setattr(playlist_mgr._repo, 'save',
                        MagicMock(side_effect=Exception("save fail")))
    ret = playlist_mgr.update_single_playlist({
        "id": "p1",
        "name": "P1",
        "files": []
    })
...
def test_update_file_duration_empty_path(playlist_mgr):
    ...
    assert playlist_mgr._duration_fetcher.update_file_duration(
        "", {"uri": ""}) is None
...
def test_update_file_duration_already_has_duration(playlist_mgr, monkeypatch):
    ...
    ret = playlist_mgr._duration_fetcher.update_file_duration(
        "f.mp3", file_item)
    assert ret == 10
...
def test_collect_files_without_duration_no_blacklist(playlist_mgr):
    ...
    uris = playlist_mgr._duration_fetcher.collect_files_without_duration(
        p1, check_blacklist=False)
