...
from core.tools.assrt_client import AssrtClient, _lang_ok, _row
...
def test_lang_ok_doc_example_korean():
    sub = {"lang": {"desc": "韩  双语", "langlist": {"langdou": True, "langkor": True}}}
    assert _lang_ok(sub, {"ko"}) is True
    assert _lang_ok(sub, {"en"}) is False
...
@patch("core.tools.assrt_client._http_get")
def test_search_by_query_maps_results(mock_http, monkeypatch):
    monkeypatch.setattr("core.tools.assrt_client.config.ASSRT_API_KEY", "test-token", raising=False)
