...
**评估目录**: `server/frontend/`
**评估日期**: 2025-01-08  
**最后更新**: 2026-01-31

---

## 📊 项目规模统计（2026-01-31 更新）

### 代码量统计

- **总源码文件数**: 约 109 个（TypeScript + Vue + CSS，含根目录配置）
- **src 下代码行数**: 约 19,907 行（.ts + .vue + .css）
- **项目规模**: **大型项目**（>10,000 行）
- **文件类型**: TypeScript (.ts) + Vue 单文件组件 (.vue) + CSS
...
│   │   ├── Timetable.vue
│   │   ├── dialogs/      # 7 个对话框/抽屉
│   │   ├── media/        # 媒体管理（6 个 Vue + 9 个 composables）
│   │   ├── my-todo/      # Chat / Info / Lottery / Score
│   │   └── tools/        # AudioConvert / AudioMerge / PdfTool / TTS 等
│   ├── components/       # 公共组件（如 MediaComponent.vue）
│   ├── composables/      # 组合函数（4 个：useAudioPlayer / useErrorHandler / useFileBrowser / useInterval）
│   ├── stores/           # Pinia（index + user）
│   ├── types/            # 类型定义（api / device / file / lottery / playlist / schedule / timetable / tools / user）
│   ├── utils/            # 工具函数（约 10 个）
│   ├── constants/        # 常量（device / media / playlist / schedule / index）
│   ├── styles/           # main.css
│   └── router/           # 路由配置
├── vite.config.ts
├── tsconfig.json
...
- **路由**: Vue Router ^4.6.4
- **状态管理**: Pinia ^3.0.4
- **UI 库**: Element Plus 2.13.1 + Vant ^3.4.3
- **样式**: Tailwind CSS ^4.1.18 + PostCSS
- **工具库**: Axios, Day.js, Lodash-es, CryptoJS, Socket.io-client
- **代码规范**: ESLint + Prettier
- **自动化**: unplugin-auto-import、unplugin-vue-components、unplugin-icons
...
**当前项目**: 约 **19,907 行**（src 下 .ts/.vue/.css），属于**大型项目**。

### 复杂度指标

- **文件数量**: 约 109 个（src 及根目录 ts/vue/css 等）
...
1. **代码质量提升** ✅
   - 从 9,211 行 JavaScript → 约 19,907 行 TypeScript/Vue（评估目录 `frontend/`）
   - 超大文件问题已解决（拆分模块化）
