...
def mi_volume():
    """获取或设置小米设备音量"""
    try:
        # GET 请求只从 query 参数获取 device_id
        # POST 请求可以从 query 参数或 JSON body 获取 device_id
        ...
        if request.method == 'GET':
            ...
            if not request.is_json:
                return _err('Content-Type must be application/json for POST requests')

            volume = request.json.get('volume') if request.json else None
            if volume is None:
