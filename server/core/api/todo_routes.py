...
def create_todo() -> ResponseReturnValue:
    ...
    try:
        ...
        schedule_data.startTs = json_data.get('start_ts')
        schedule_data.endTs = json_data.get('end_ts')
        schedule_data.allDay = json_data.get('all_day', 1)
        schedule_data.reminder = json_data.get('reminder', 0)
        schedule_data.repeat = json_data.get('repeat', 0)
        schedule_data.repeatData = json_data.get('repeat_data', {})
        schedule_data.repeatEndTs = json_data.get('repeat_end_ts')
        schedule_data.color = json_data.get('color', 0)
        schedule_data.priority = json_data.get('priority', -1)
        schedule_data.groupId = json_data.get('group_id', -1)
        schedule_data.order = json_data.get('order_idx', 0)
        schedule_data.score = json_data.get('score')
        schedule_data.userId = json_data.get('user_id')
