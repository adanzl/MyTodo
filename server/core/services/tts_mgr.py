...
- 分析文章（调用 txt_ali 大模型分析文字，仅保存结构化分析结果到任务，不修改原始文本）
...
from core.ai.txt_ali import TxtAli
...
class TTSTask(TaskBase):
    ...
    # 文本分析结果（例如调用 txt_ali 返回的 JSON 结构），由分析文章任务写入
...
class TTSMgr(BaseTaskMgr[TTSTask]):
    ...
    def _run_tts_task(self, task: TTSTask) -> None:
        ...
        # 错误信息（用于在回调中传递错误）
        error_info: dict[str, Exception | None] = {'error': None}
        # 已生成字数统计
        generated_chars = 0
        # 音频数据统计
        audio_data_size = 0
        ...
        try:
            # 创建 TTS 客户端并设置参数
            # 注意：tts_ali 使用 WebSocket 连接，不依赖 SDK
            ...
            if is_stopped:
                ...
                # 更新任务状态为停止
                with self._task_lock.gen_wlock():
                    _task = self._get_task(task.task_id)
                    if _task:
                        # 如果任务已经被 stop_task 设置为停止状态，则不再更新
                        if _task.status == TASK_STATUS_FAILED and _task.error_message == '任务已被用户停止':
                            pass  # 状态已更新，跳过
                        else:
                            # 任务被停止，标记为失败但错误信息明确说明是停止
                            self._update_task_status(_task.task_id, TASK_STATUS_FAILED, '任务已被用户停止')
...
*具体格式与要求严格遵循以下规范*：
    ...
    - 优美词汇(words)：从文章中选取 7到8 个贴合文意、有积累价值的词语，每个词不要超过4个字；
