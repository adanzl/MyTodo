...
class LotteryMgr:
    ...
    def do_lottery(self, user_id: int, cate_id: int) -> Dict[str, Any]:
        ...
        user_data = self._db.get_data('t_user', user_id,
                                      "id,score,wish_progress,wish_list")
        if user_data['code'] != 0:
        ...
        if cate_cost is None:
            return {
                "code": -1,
                "msg":
                "No lottery data" if cate_id == 0 else "Category not found",
            }
    ...
    def _get_cost_and_wish_threshold(self, cate_id: int) -> tuple:
        ...
        if cate_id == 0:
            ...
            t = max(1, int(t)) if isinstance(t, (int,
                                                 float)) else default_threshold
            return fee, t
    ...
    def _parse_wish_list_ids(self, wish_list_str: str) -> list:
        ...
        if not isinstance(arr, list):
            return []
        return [
            int(x) for x in arr if isinstance(x, (int, float)) and int(x) == x
        ]
    ...
    def _get_wish_pool(self, user_id: int, cate_id: int,
                       wish_list_str: str) -> list:
    ...
    def _commit_win(self, user_id: int, cate_cost: Any,
                    gift: Dict[str, Any]) -> Dict[str, Any]:
        """扣库存、扣积分并返回中奖结果。"""
        gid = gift.get('id')
    ...
    def undo_lottery(self, history_id: int) -> Dict[str, Any]:
        ...
        # 礼物库存 +1
        gift_res = self._db.get_data('t_gift', int(out_key), 'id,stock')
    ...
    def get_gift_avg_cost(
        ...
        resp = self._db.get_list('t_gift', 1, 1000,
                                 ['cost', 'cate_id', 'stock'], conditions
                                 or None)
        ...
        for g in gifts:
            ...
            s = g.get('stock')
            if not isinstance(c,
                              (int, float)) or not isinstance(s, (int, float)):
                continue
            ...
            by_cate[cate_id] = (prev_w + cost_val * stock_val,
                                prev_s + stock_val)
        ...
        by_category = []
        for cate_id, (cate_weighted, cate_stock) in sorted(by_cate.items(),
                                                           key=_sort_key):
