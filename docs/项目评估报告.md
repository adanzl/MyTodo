...
│   └── PageSchedule.ts
│   ├── PageCalendar.vue  # 日历
│   ├── PageTimetable.vue # 课表
│   ├── PagePic.vue       # 图片
│   ├── PageSave.vue      # 收藏/存档
│   ├── page-chat/           # 聊天模块（AI / 房间 / TTS 任务）
│   │   ├── PageChat.vue
│   │   ├── TabAiChat.vue
│   │   ├── TabChatRoom.vue
│   │   └── TabTtsTasks.vue
...
| 模块 | 说明 | 主要文件 |
| --- | --- | --- |
| **日程** | 日程管理、弹窗、子任务 | page-schedule/PageSchedule.*、SchedulePopModal.*、SubtaskPopModal |
| **日历** | 日历展示与联动 | PageCalendar、CalendarTab、CalendarCover |
| **课表** | 课表展示 | PageTimetable |
| **聊天** | AI 对话、聊天室、TTS 任务 | page-chat/* |
| **抽奖** | 抽奖、历史、商店 | page-lottery/* |
| **通用** | 图片、收藏、设置、主题 | PagePic、PageSave、GConfig、ColorMgr 等 |
...
- **分层清晰**：views / components / api / types / utils 职责分明，页面与逻辑、类型、接口分离（如 PageSchedule.ts + .vue）。
...
- **可维护性** — **当前得分：4.5/5**
  1. **单文件拆子组件/composable**（提升空间高）：单文件体量过大的视图按职责拆成子组件或 composable，便于单测与复用。**需处理文件**（单文件 ≥300 行或同页面 .vue+.ts 合计 ≥500 行）：
     - `src/views/PageTimetable.vue`（865 行）
     - `src/views/page-chat/PageChat.vue`（667 行）
     - `src/views/page-chat/TabTtsTasks.vue`（795 行）
     - `src/views/page-schedule/PageSchedule.vue` + `PageSchedule.ts`（209 + 551 = 760 行）
     - `src/components/SchedulePopModal.vue` + `SchedulePopModal.ts`（383 + 472 = 855 行）
     - `src/views/TabsPage.vue`（493 行）
     - `src/views/page-lottery/PageLottery.vue`（391 行）
