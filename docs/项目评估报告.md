...
| 模块 | 说明 | 主要文件 |
| --- | --- | --- |
| **日程** | 日程管理、弹窗、子任务 | PageSchedule.*、SchedulePopModal.*、SubtaskPopModal |
...
- **可维护性** — **当前得分：4.5/5**
  1. **单文件拆子组件/composable**（提升空间高）：单文件体量过大的视图按职责拆成子组件或 composable，便于单测与复用。**需处理文件**（单文件 ≥300 行或同页面 .vue+.ts 合计 ≥500 行）：
     ...
     - `src/views/chat-page/tab-tts-tasks.vue`（795 行）
     - `src/views/chat-page/page-chat.vue`（667 行）
     - `src/views/PageSchedule.vue` + `PageSchedule.ts`（209 + 551 = 760 行）
     - `src/components/SchedulePopModal.vue` + `SchedulePopModal.ts`（383 + 472 = 855 行）
     - `src/views/TabsPage.vue`（493 行）
     - `src/views/lottery-page/PageLottery.vue`（391 行）
