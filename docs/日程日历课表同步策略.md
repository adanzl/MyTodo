...
- **进入 Tab 时拉取**：日程页（`page-schedule/PageSchedule`）、日历页（PageCalendar）在 `onIonViewDidEnter` 时调用 `refreshAllData(scheduleListId)`，即每次进入该 Tab 拉取一次 `getSave(id)`。
...
| 模块 | 拉取/刷新 | 保存 | 事件 |
| --- | --- | --- | --- |
| 日程 | `page-schedule/PageSchedule.ts`：`refreshAllData`、`onIonViewDidEnter` | `page-schedule/PageSchedule.ts`：`doSaveUserData` → `setSave`；`CalendarCover.vue`：`setSave` | `C_EVENT.UPDATE_SCHEDULE_GROUP` |
