...
- **进入 Tab 时拉取**：日程页（TabSchedulePage）、日历页（TabCalendarPage）在 `onIonViewDidEnter` 时调用 `refreshAllData(scheduleListId)`，即每次进入该 Tab 拉取一次 `getSave(id)`。
...
- **进入页面时拉取**：TabTimetablePage 在 `onMounted` 时调用 `loadTimetableData()`，即 `getRdsData("t_timetable", 0)` 一次。
...
| 模块 | 拉取/刷新 | 保存 | 事件 |
| --- | --- | --- | --- |
| 日程 | `TabSchedulePage.ts`：`refreshAllData`、`onIonViewDidEnter` | `TabSchedulePage.ts`：`doSaveUserData` → `setSave`；`CalendarCover.vue`：`setSave` | `C_EVENT.UPDATE_SCHEDULE_GROUP` |
| 日历 | `TabCalendarPage.vue`：`refreshAllData`、`onIonViewDidEnter` | 同日程（共用 `getSave`/`setSave`） | 同上 |
| 课表 | `TabTimetablePage.vue`：`loadTimetableData`、`onMounted`、`refreshData` | `TabTimetablePage.vue`：`setRdsData("t_timetable", 0, ...)` | 无 |
