...
| 操作 | 确认方式 | 反馈方式 | 代码位置 |
| --- | --- | --- | --- |
| **删除日程** | 弹窗「确认删除」+ 文案「确定要删除日程「xxx」吗？删除后不可恢复。」 | 确认后 Toast「已删除日程」 | `page-schedule/PageSchedule.ts`（btnScheduleRemoveClk、onDelSchedulerConfirm） |
...
| 抽奖 | `api/lottery.ts`（doLottery、getLotteryData） | `views/page-lottery/*` |
